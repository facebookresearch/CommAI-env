...
$atomic_task_switching_episodes_count = shift;
...
END_TASK_SPECIFICATION
    ...
    "${composition_type}LookupTaskR2D${i}_${j}": {
      "type": "tasks.micro.$lookup_table_name.${composition_type}LookupTaskR2D${i}_${j}"
    },
END_TASK_SPECIFICATION
    ...
    "${composition_type}LookupTaskR2D${control_i}_${control_j}": {
      "type": "tasks.micro.$lookup_table_name.${composition_type}LookupTaskR2D${control_i}_${control_j}"
    },
END_TASK_SPECIFICATION
...
$begin = $end+1;
$end += $atomic_task_switching_episodes_count;

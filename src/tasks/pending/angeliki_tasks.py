...
from core.task import Task, on_start, on_message
import random
import re
...
class RepeatingTaskLogical2Objects(Task):
    def __init__(self):
        ...
        self.objects = [gl_objects[random.getrandbits(1)],
                        gl_objects[random.getrandbits(1)]]
        ...
        if self.negation_2 == "dont":
            self.answer.pop(0)
            message = " ".join([self.negation_1, "say", self.objects[0],
                                self.logic_operator, self.negation_2, "say",
                                self.objects[1]])
        else:
            message = " ".join([self.negation_1, "say", self.objects[0],
                                self.logic_operator, self.negation_2,
                                self.objects[1]])
    ...
    def on_message(self, event):
        ...
        if not m[1] in self.delim_answer:
            ...
            overlap = len(set(self.answer) & set([m[0], m[2]]))
            overlap /= len(self.answer)
            correct = (self.logic_operator == "and" and overlap == 1) or \
                      (self.logic_operator == "or" and overlap == 0.5)
            if correct:
                self.set_reward(1)
            else:
                self.set_reward(0)
